...
Jira Project Dashboard and Speakers Collective currently use host-configured agent integrations
and are included here so the runtime image has one source boundary. CAIPE deployment configuration
...
| Speakers Collective | `/apps/speakers-collective` | 3043 | `agent-speakers-collective` | Governed CFP and speaker workflow |
...
The smoke test runs all four servers with JWT verification disabled on loopback only. Production
...
with `agentic-apps-`. The image contains the shared runtime libraries and all seven application
