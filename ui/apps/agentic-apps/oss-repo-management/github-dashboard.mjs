...
export async function fetchGitHubRepoDashboard({
  ...
  if (Number.isNaN(currentTime.getTime())) throw new GitHubDashboardError("The report timestamp is invalid.", { status: 400, code: "invalid_timestamp" });
  const staleBefore = new Date(currentTime.getTime() - boundedStaleDays * 86_400_000).toISOString().slice(0, 10);
  const trendStart = startOfUtcWeek(new Date(currentTime.getTime() - (TREND_WEEKS - 1) * 7 * 86_400_000));
  ...
  return {
    ...
    trends: {
      ...
      commitsPerWeek: aggregateWeekly(commits, (item) => item?.commit?.author?.date || item?.commit?.committer?.date, trendStart, currentTime),
      pullRequestsPerWeek: aggregateWeekly(recentPulls, (item) => item?.created_at, trendStart, currentTime),
      engagementPerWeek: aggregateWeeklyWeighted(recentIssues, (item) => item?.created_at, (item) => 1 + Number(item?.comments || 0), trendStart, currentTime),
